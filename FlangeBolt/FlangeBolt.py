...
UTS[MPa] = 1100
YS[MPa] = 900
Bolt Size Metric M = 6
Thread Minimum Dia.[mm] = 5.24
Shank Length[mm] = 
Thread Length[mm] = 
Thread Stretch Ratio[mm] = 0.5
Proof Load Ratio = 0.9
...
Transform Location = Yes
Transform Rotation = Yes
...
with open('geom.json', 'w') as fp:
    json.dump(outdata, fp)

#plot results
plt.clf()
plt.figure()
df.plot(x='TCrpm', y=['P1Tabs', 'P2Tabs','P1Cabs','P2Cabs','PtCabs','PtTabs'])
...
plt.clf()
plt.figure()
df.plot(x='Time', y=['P1Tabs', 'P2Tabs','P1Cabs','P2Cabs','PtCabs','PtTabs'])
plt.yticks(np.linspace(0,600,20))
plt.ylabel('Pressure [kPaA]')
plt.xlabel('Time[s]')
...
plt.yticks(np.linspace(-250,250,20))
plt.ylabel('Thrust [N]')
plt.xlabel('rpm')
